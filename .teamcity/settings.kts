...
project {
    ...
    params {
        param("travel_planner-api-domain_name", "travelplanner.solutions")
        param("travel_planner-project_route", "/home/vpcuser/TravelPlaner")
    }
...
object BuildApi : BuildType({
    ...
    steps {
        sshExec {
            ...
            commands = """
                cd %travel_planner-project_route%
                rm -rf backend
            """.trimIndent()
            targetUrl = "%travel_planner-api-domain_name%:%travel_planner-project_route%/backend"
        ...
        sshExec {
            ...
            id = "Create_env_prod"
            commands = """
                cd %travel_planner-project_route%/backend
                cat <<'EOF' > .env.prod
    ...
    steps {
        sshExec {
            ...
            commands = """
                ...
                rm -rf frontend
            """.trimIndent()
            targetUrl = "%travel_planner-api-domain_name%:%travel_planner-project_route%/frontend"
            authMethod = uploadedKey {
                username = "vpcuser"
                key = "travelplaner-ibmc-vpc.prv"
            }
        ...
        sshExec {
            ...
            id = "Docker_Compose"
            commands = """
                cd %travel_planner-project_route%/frontend
                sudo docker compose --env-file .env.prod -f docker-compose-prod.yml up --build -d
            """.trimIndent()
            targetUrl = "%travel_planner-api-domain_name%:%travel_planner-project_route%/frontend"
            authMethod = uploadedKey {
