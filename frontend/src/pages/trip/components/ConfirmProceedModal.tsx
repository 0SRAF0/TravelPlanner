    ...
    <Modal isOpen={isOpen} onClose={onClose}>
      <div className="space-y-6">
        <div className="w-14 h-14 rounded-full bg-amber-100 text-amber-700 flex items-center justify-center mx-auto">
          <svg className="w-7 h-7" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            <path
              ...
              strokeWidth={2}
              d="M12 9v4m0 4h.01M10.29 3.86l-7.4 12.84A2 2 0 004.53 20h14.94a2 2 0 001.73-3.3L13.8 3.86a2 2 0 00-3.51 0z"
            />
          </svg>
        </div>
            ...
            <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
              ...
              <div className="flex flex-wrap gap-2">
                {pendingMembers.map((m, idx) => (
                  <div
                    key={`${m.name}-${idx}`}
                    className="flex items-center gap-2 bg-white border border-amber-200 rounded-full pr-3 pl-1 py-1 shadow-sm"
                  >
                    {m.picture ? (
                      <img
                        src={m.picture}
        ...
        <div className="flex gap-3 justify-end">
          ...
          <Button text={isProcessing ? 'Processing...' : 'OK'} onClick={onConfirm} size="base" />
        </div>
      </div>
    </Modal>
  );
