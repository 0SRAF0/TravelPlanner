...
from app.db.database import get_activities_collection, get_database, get_preferences_collection
...
async def submit_preferences(
    ...
    preferred_categories: list[str] | None = Query(None, description="Preferred categories"),
):
    """
    ...
    if not preferences:
        raise HTTPException(status_code=404, detail=f"No preferences found for trip_id: {tid}")
    ...
    try:
        ...
        try:
            ...
            if trip_doc is None:
                trip_doc = await db.trips.find_one({"trip_id": tid}) or await db.trips.find_one(
                    {"_id": tid}
                )
            if trip_doc:
            ...
            print(
                f"[submit_preferences] Warning: could not resolve destination for trip {tid}: {e}"
            )
        ...
        if destination:
            ...
            if activities:
                ...
                try:
                    ...
                    if docs:
                        ...
                        print(
                            f"[submit_preferences] Saved {len(res.inserted_ids)} activities for trip={tid}"
                        )
                    else:
            ...
            print(
                "[submit_preferences] No destination provided; skipping destination research agent invocation."
            )
    except Exception as e:
    ...
    return APIResponse(
        code=0,
        msg="ok",
        data={"success": True, "trip_id": tid, "preferences_ingested": ingested_count},
    )
