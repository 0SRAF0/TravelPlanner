...
async def google_auth(
    ...
    print("=" * 50)
    print(
        f"Authorization Code: {code[:20]}..." if len(code) > 20 else f"Authorization Code: {code}"
    )
    print("=" * 50 + "\n")
    ...
    try:
        ...
        async with httpx.AsyncClient() as client:
            ...
            if token_response.status_code != 200:
                raise HTTPException(
                    status_code=400, detail=f"Failed to exchange code: {token_response.text}"
                )
        ...
        print("=" * 50)
        print(
            f"JWT Token: {jwt_token[:30]}...{jwt_token[-20:] if len(jwt_token) > 50 else jwt_token[30:]}"
        )
        ...
        return APIResponse(
            code=0,
            msg="ok",
            data={"access_token": jwt_token, "token_type": "bearer", "user": user_info},
        )
